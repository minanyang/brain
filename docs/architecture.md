...
- `brief.md` is the bridge back: when `inject_brief` is `true` in `~/.brain/config.json`, the plugin's `SessionStart` hook adds the routed vault's brief to every session (it passes the secret gate first). The default is `false`: injection costs tokens on every session, and a wrong synthesized fact would reach every conversation until lint catches it — so the switch is flipped once the owner has read a few weeks of the wiki and trusts it. Resolving a round of conflicts by hand is that point. The same switch adds a **session map**: the wiki pages that the last 30 days of sessions in the current directory (or below it) fed, most cited first, capped at 20, each with its `index.md` line. The instruction that heads the map was measured, not assumed. Its first version said only "read the page before stating facts about what it covers", and agents did: they took a status from a page that had gone stale instead of checking git, and took a decision's reasons from the page's summary instead of opening the spec it cited. The instruction now says what a page is: notes from past sessions, dated. It sends status questions to the system itself and reason questions to the cited primary document. The brief says who the user is; the map says which pages this session is likely to need. Without it nothing points a session at the wiki — measured on the first vault: 136 distills and 14 ingests, zero queries — because an agent does not query for a fact it does not know it lacks. `index.md` is not injected whole: at a hundred pages it is 24 KB.
