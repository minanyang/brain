...
Automatic ingest is opt-in through `auto_ingest.enabled`, because it spends model budget without being asked and rewrites `brief.md`, which every later session reads. Its thresholds (`min_pending`, `max_age_hours`, `min_interval_hours`, `model`) live beside it in `~/.brain/config.json`. It runs the ingest skill in a headless session whose tools are the skill's own `allowed-tools` — the four scripts and the file tools, no bare `Bash` — so an unattended run cannot reach `git` directly and `finish.sh` stays the only write path. **It resolves no conflict at all**: nobody is there to decide, it has no `git` or cloud access to settle even a question of fact, and a guess recorded as a decision leaves no trace to find later, so contradictions stay `[open]` for the next human-driven run.
...
- `brief.md` is the bridge back: when `inject_brief` is `true` in `~/.brain/config.json`, the plugin's `SessionStart` hook adds the routed vault's brief to every session (it passes the secret gate first). The default is `false`: injection costs tokens on every session, and a wrong synthesized fact would reach every conversation until lint catches it — so the switch is flipped once the owner has read a few weeks of the wiki and trusts it. Resolving a round of conflicts by hand is that point. The same switch adds a **session map**: the wiki pages that the last 30 days of sessions in the current directory (or below it) fed, most cited first, capped at 20, each with its `index.md` line and the instruction to read the page before stating facts about what it covers. The brief says who the user is; the map says which pages this session is likely to need. Without it nothing points a session at the wiki — measured on the first vault: 136 distills and 14 ingests, zero queries — because an agent does not query for a fact it does not know it lacks. `index.md` is not injected whole: at a hundred pages it is 24 KB.
