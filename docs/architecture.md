...
| Deterministic scripts: transcript extractor, distill runner, memory sync, secret gate, vault init, status, routing (`resolve-vault.sh`), ingest prep, auto-ingest, `finish.sh` (gate → mark sources → regenerate `index.md` and `brief.md` → log → commit → tag, shared by every op), `index.sh`, `lint.sh`, `ref.sh`, `verify.sh` (re-checks status markers against git), `git-read.sh` (read-only git for agents) | this repo (`scripts/`) | called by skills and hooks via `${CLAUDE_PLUGIN_ROOT}` |
